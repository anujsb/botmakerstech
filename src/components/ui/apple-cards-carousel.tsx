    ...
    >
      <div className="relative w-full ">
        ...
        <div className="flex justify-end gap-2 mr-10">
          <Button
            className="relative z-40 h-14 w-14 rounded-full bg-gray-100 flex items-center justify-center disabled:opacity-50"
            onClick={scrollLeft}
            disabled={!canScrollLeft}
          >
            <IconArrowNarrowLeft className="h-10 w-10 text-black" />
          </Button>
          <Button
            className="relative z-40 h-14 w-14 rounded-full bg-gray-100 flex items-center justify-center disabled:opacity-50"
            onClick={scrollRight}
          ...
          >
            <IconArrowNarrowRight className="h-20 w-20 text-black" />
          </Button>
        </div>
      </div>
    </CarouselContext.Provider>
  ...
  return (
    <>
          ...
          <div className="fixed inset-0 h-screen z-50 overflow-auto">
            ...
            >
              ...
              <motion.p
                ...
                className="text-base font-medium text-neutral-700 mt-4"
              >
                {card.title}
              </motion.p>
      ...
      <motion.button
        ...
        layoutId={layout ? `card-${card.title}` : undefined}
        onClick={handleOpen}
        className="rounded-3xl bg-gray-100 h-80 w-64 md:h-[40rem] md:w-[36rem] overflow-hidden flex flex-col items-center justify-end relative z-10"
      >
        ...
        <div className="relative z-40 p-8 backdrop-blur-sm bg-black/50 w-full ">
          ...
          <motion.p
            layoutId={layout ? `title-${card.title}` : undefined}
            className="text-white text-sm md:text-base font-medium max-w-xs text-center [text-wrap:balance] font-sans mt-2 "
          >
            {card.title}
          </motion.p>
        </div>
